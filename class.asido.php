...
Class Asido {
	...
	*/
	function driver($driver_name) {
		...
		// class exists ?
		//
		if (class_exists($c = asido::driver_classname($driver_name))) {
			asido::driver(new $c);
			return true;
			}
		...
		//
		if (!$fp = @fopen(
				$f = asido::driver_filename($driver_name), 'r', 1)
			) {
			trigger_error(
				sprintf(
	...
	* @static
	*/
	function driver_filename($driver_name) {
	...
	* @static
	*/
	function driver_classname($driver_name) {
